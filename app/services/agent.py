...
from app.services.tools import tool_list, ToolSetName
...
def get_resource_from_agent_response(response: Dict[str, Any] )-> List[dict]:
    if steps := response.get('intermediate_steps') :
        last_step = steps[-1]
        if last_step[0].tool == ToolSetName.DEEP_SEARCH_AND_FILTER:
            return [doc.metadata for doc in last_step[1]] 
        elif last_step[0].tool == ToolSetName.SHALLOW_SEARCH_RESULT:
            return last_step[1]
        elif last_step[0].tool == ToolSetName.TAVILY_SEARCH_TOOLS:
            return last_step[1]
